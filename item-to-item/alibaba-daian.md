...
实现方面，将隐式意图表征和trigger-target相似度，经过PepNet的门控网络 +多层MLP，获得隐式权重，跟隐式意图表征做hadmard积，获得加权隐式意图表征。相似的，将显式意图表征和trigger-target相似度，经过同样的网络，获得加权显式意图表征。最后，这两个表征会和用户、Trigger、target等特征一起拼接，喂给CTR预测头（基模型DIN的MLP）。
