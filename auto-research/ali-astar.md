...
## 3 落地：数据侧和模型侧接成闭环
线上workflow六步：Astar根据当前系统状态生成一批候选方向 → **Reward Model打分排序，挑最有希望的往下走** → Code Agent（Qwen/Claude这类通用模型）翻译成可运行代码 → 训练+离线评估 → 赢了就推上线 → 结果走一遍去噪流水线抽成结构化样本，回流继续训Astar。

第二步是整条链路的经济性所在：真验证一个方向要按天算，RM打分是秒级，而它的AUC有0.8487——**先用秒级的筛掉大部分，再把按天算的算力花在活下来的那几个上**。RL阶段训出来的这个RM，在线上是复用的，不是训完就扔。

最后一步走的就是数据侧那套去噪流水线——**每一轮线上迭代都在给自己造训练数据**。作者强调这个闭环是必须的：**静态的Astar会掉队**，系统一旦演进到训练数据没覆盖的状态，建议就过时了。

（多说一句：论文的Figure 6和3.4节的枚举其实都漏了RM这一环，图上是Astar直接连Code Agent。但caption写着"six stages"底下只列了五条，而1节和C4段都明确说了RM要在推理时"rapid ranking of generated evolution direction candidates"。这里按实际逻辑补上。）
