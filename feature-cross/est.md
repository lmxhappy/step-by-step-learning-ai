...
本文可以认为是OneTrans 2.0，核心区别就是行为-行为交互从 dense causal self-attention 变成了 content-based sparse attention。两者最终都是让非行为特征能看到行为序列。金字塔 vs sparse attention，不见得谁的计算量更小。
